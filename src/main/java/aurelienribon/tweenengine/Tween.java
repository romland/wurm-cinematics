...
public final class Tween extends BaseTween<Tween> {
	...
	public Tween build() {
		...
		if (accessor != null) combinedAttrsCnt = accessor.getValues(this, target, type, accessorBuffer);
	...
	protected void initializeOverride() {
		...
		accessor.getValues(this, target, type, startValues);
	...
	protected void updateOverride(int step, int lastStep, boolean isIterationStep, float delta) {
		...
		if (!isIterationStep && step > lastStep) {
			accessor.setValues(this, target, type, isReverse(lastStep) ? startValues : targetValues);
			return;
		}

		if (!isIterationStep && step < lastStep) {
			accessor.setValues(this, target, type, isReverse(lastStep) ? targetValues : startValues);
			return;
		}
		...
		if (duration < 0.00000000001f && delta > -0.00000000001f) {
			accessor.setValues(this, target, type, isReverse(step) ? targetValues : startValues);
			return;
		}

		if (duration < 0.00000000001f && delta < 0.00000000001f) {
			accessor.setValues(this, target, type, isReverse(step) ? startValues : targetValues);
			return;
		}
		...
		accessor.setValues(this, target, type, accessorBuffer);
	}
	...
	protected void forceStartValues() {
		if (target == null) return;
		accessor.setValues(this, target, type, startValues);
	}

	@Override
	protected void forceEndValues() {
		if (target == null) return;
		accessor.setValues(this, target, type, targetValues);
	}
